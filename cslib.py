...
def fetch_ts(data_dir, clean=False):
    ...
    print('ts_data_dir: ', ts_data_dir, os.path.exists(ts_data_dir))
    
    if clean:
        shutil.rmtree(ts_data_dir)
    if not os.path.exists(ts_data_dir):
        os.makedirs(ts_data_dir)
    ...
    print('loading data with fetch_data: ', df.head())
    ...
    for key, item in dfs.items():
        print(key, item, os.path.join(ts_data_dir,"ts-"+key+".csv"))
